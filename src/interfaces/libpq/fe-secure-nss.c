 ...
 * NOTES
 *
 *	  Relevant links for setting up TLS:
 *
 ...
 *
 *	  Creating an NSS database and importing the client cert, private key
 *	  and root cert into it:
 *
 *	  mkdir /home/danielg/dev/postgresql/obj/nss/ssldb3
 *	  certutil -d "sql:/home/danielg/dev/postgresql/obj/nss/ssldb3" -N --empty-password
 *	  certutil -d "sql:/home/danielg/dev/postgresql/obj/nss/ssldb3" -A -n danielg -i /home/danielg/dev/postgresql/obj/openssl/certs/client.crt -t "CT,C,C"
 *	  certutil -d "sql:/home/danielg/dev/postgresql/obj/nss/ssldb3" -A -n root -i /home/danielg/dev/postgresql/obj/openssl/certs/ca.crt -t "CT,C,C"
 *	  openssl pkcs12 -export -out keys/client.pfx -inkey keys/client.key -in certs/client.crt -certfile certs/ca.crt
 *    pk12util -i /home/danielg/dev/postgresql/obj/openssl/keys/client.pfx -d ./ssldb3/
 *
 *
 *    ./bin/psql "port=5432 sslmode=require dbname=postgres host=localhost cert_database=/home/danielg/dev/postgresql/obj/nss/ssldb3 sslcert=danielg"
 *
 *    ./bin/psql "port=5434 sslmode=verify-ca dbname=postgres host=127.0.0.1 sslcert=/home/danielg/dev/postgresql/obj/openssl/certs/client.crt sslkey=/home/danielg/dev/postgresql/obj/openssl/keys/client.key sslrootcert=/home/danielg/dev/postgresql/obj/openssl/certs/ca.crt"
 *
 *
 * QUESTIONS
 *
 *	  * Need to support bare PEM files on disk?
 *	  * Need to support all sslmodes? Only verify-full and not verify-ca?
 *	  * Do we need/want client TLS library in pg_stat_ssl? (I'd say so..)
 *
 *-------------------------------------------------------------------------
 */
...
pgtls_open_client(PGconn *conn)
{
	...
	if (!conn->cert_database && conn->sslmode[0] == 'v')
	{
		...
		{
			printfPQExpBuffer(&conn->errorMessage,
							  libpq_gettext("WARNING: unable to load NSS trust module"));
			return PGRES_POLLING_FAILED;
		}
	}
	...
	{
		int ssl_min_ver = ssl_protocol_version_to_nss(conn->sslminprotocolversion);
	...
	{
		int ssl_max_ver = ssl_protocol_version_to_nss(conn->sslmaxprotocolversion);
	...
	{
		printfPQExpBuffer(&conn->errorMessage,
						  libpq_gettext("unable to connect: %s"),
						  				pg_SSLerrmessage(PR_GetError()));
		return PGRES_POLLING_FAILED;
	}
...
pg_cert_auth_handler(void *arg, PRFileDesc *fd, PRBool checksig, PRBool isServer)
{
	 ...
	 * validation, so return immediately.
	 */
	if (conn->sslmode[0] != 'v')
		return SECSuccess;
...
pg_bad_cert_handler(void *arg, PRFileDesc *fd)
{
	...
	/*
	 * For sslmode verify-ca and verify-full, a failed certificate validation
	 * is an error. For allow, require and prefer we allow this however, as
	 * self-signed server certificates would otherwise be refused.
	 */
	if (conn->sslmode[0] == 'v')
		return SECFailure;
...
PQsslStruct(PGconn *conn, const char *struct_name)
{
	 ...
	 *
	 * TODO: decide if this really should be nss in lowercase, or perhaps
	 * even "libnss". There isn't a strong brandname for nss like for OpenSSL
