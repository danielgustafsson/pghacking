 ...
 *
 * TODO:
 *	- Document NSS/NSPR combination, which is what etc
 *
 * IDENTIFICATION
...
/*
 * Lists containing NSS objects which aren't destroyed automatically, but
 * which need to be explicitly freed to reclaim the memory allocated by NSS
 * when shutting down the connection.
 */
static List *crl_list = NIL;
...
static uint16 ssl_protocol_version_to_nss(int v, const char *guc_name,
										  int loglevel);
static bool pg_load_crl(const char *filename);
...
{
	SECStatus			status;
	SSLVersionRange		desired_sslver;
	...
	if (!nss_context)
		ereport(FATAL,
				(errmsg("unable to initialize NSS: %s",
				 		pg_SSLerrmessage(PR_GetError()))));
	 ...
	 * the domestic policy. TODO: this really isn't tested very well.
	 *
	 * Also TODO: there needs to be a default set of ciphers in guc.c for nss
	 * much like how we handle OpenSSL, but for now let's just ensure we are
	 * dealing with the default of none.
	 */
	if (SSLCipherSuites && pg_strcasecmp(SSLCipherSuites, "none") != 0)
	{
		char		   *ciphers, *c, *b;
		char		   *sep = ":;, ";
		PRUint16		ciphercode;
	...
	/*
	 * Load the CRL file if configured
	 */
	if (ssl_crl_file[0])
	{
		if (!pg_load_crl(ssl_crl_file))
			goto error;
	}
	...
	/*
	status = SSL_BadCertHook(model, pg_bad_cert_handler, (void *) port);
	if (status != SECSuccess)
		ereport(ERROR,
				(errmsg("unable to install authcert hook: %s",
						pg_SSLerrmessage(PR_GetError()))));
	*/
	...
	peerCert = SSL_PeerCertificate(port->pr_fd);

	if (peerCert)
	{
		elog(LOG, "XXX: got certificate: Subject:%s; Issuer:%s",
...
be_tls_destroy(void)
{
	...
	/* TODO: Unload any modules with static storage */

	/*
	 * Free all entries in the CRL cache, there is no API call to do this for
	 * the entire cache so we must manually handle it.
	 */
	foreach(lc, crl_list)
	{
		SECItem *crlitem = lfirst(lc);

		SECITEM_FreeItem(crlitem, PR_TRUE);
	}
}
...
pg_bad_cert_handler(void *arg, PRFileDesc *fd)
{
	...
	elog(LOG, "XXX: pg_bad_cert_handler();");


	certificate = SSL_PeerCertificate(port->pr_fd);
	if (certificate)
	{
		elog(LOG, "XXX: pg_bad_cert_handler(); got certificate from peer");
	}

	/*
	 * TODO: implement me.. for now we'll happily accept any cert, but once
	 * handshake works properly then this will need to be revisited.
	 */
	return SECSuccess;
}
#endif
...
/*
 * init_iolayer
 *				Initialize a new NSPR IO layer
 *
 * TODO: Document layers and why we need to do this.
 */
static PRFileDesc *
 ...
 */
static uint16
ssl_protocol_version_to_nss(int v, const char *guc_name, int loglevel)
{
	switch(v)
	{
		/*
...
/*
 * TODO: this needs a lot of testing, since it's likely not working..
 */
static bool
pg_load_crl(const char *filename)
{
	FILE		   *crlfile;
	struct stat		stat_buf;
	int				nbytes;
	SECItem			crldata = { 0, NULL, 0 };
	char		   *buf;
	CERTSignedCrl  *crl;
	...
	if (stat(filename, &stat_buf) != 0)
		ereport(ERROR,
				(errcode(ERRCODE_CONFIG_FILE_ERROR),
				 errmsg("could not read CRL file \"%s\": unable to open",
						filename)));

	crlfile = AllocateFile(filename, "r");
	if (!crlfile)
	{
		ereport(ERROR,
				(errmsg("could not open CRL file \"%s\": %m", filename)));
		return false;
	}

	buf = palloc(stat_buf.st_size);

	if (!SECITEM_AllocItem(NULL, &crldata, stat_buf.st_size + 1))
	{
		ereport(ERROR,
				(errmsg("out of memory: %s", pg_SSLerrmessage(PR_GetError()))));
		return false;
	}

	nbytes = fread(buf, 1, stat_buf.st_size, crlfile);
	FreeFile(crlfile);
	if (nbytes != stat_buf.st_size)
	{
		ereport(ERROR,
				(errmsg("could not read from CRL file \"%s\": %m", filename)));
		return false;
	}

	memcpy(crldata.data, buf, stat_buf.st_size);

	certdb = CERT_GetDefaultCertDB();

	/*
	 * See if the CRL has already been cached the DB, if so then we can skip
	 * doing any more work.
	 */
	crl = SEC_FindCrlByDERCert(certdb, &crldata, 0);
	if (crl)
	{
		SEC_DestroyCrl(crl);
		SECITEM_FreeItem(&crldata, PR_FALSE);
		return true;
	}

	/*
	 * There is no way to retrieve the CRL entries from the cache once added,
	 * so the caller must keep a reference to anything added in order to be
	 * able to destroy it on the way out.
	 */

	if (CERT_CacheCRL(certdb, &crldata) == SECSuccess)
	{
		crl_list = lappend(crl_list, (void *) &crldata);
	}
	else
	{
		SECITEM_FreeItem(&crldata, PR_TRUE);
		ereport(ERROR,
				(errmsg("could not set CRL entry: %s",
						pg_SSLerrmessage(PR_GetError()))));
		return false;
	}
...
pg_SSLerrmessage(PRErrorCode errcode)
{
	...
	/* TODO: this should probably use a StringInfo instead.. */
