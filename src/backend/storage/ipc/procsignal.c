...
static void ProcessBarrierChecksumInProgress(void);
...
ProcessProcSignalBarrier(void)
{
	 ...
	 */
	if (BARRIER_SHOULD_CHECK(flags, PROCSIGNAL_BARRIER_CHECKSUM_INPROGRESS))
	{
		ProcessBarrierChecksumInProgress();
	}
...
static void
ProcessBarrierChecksumInProgress(void)
{
	AbsorbChecksumsInProgressBarrier();
}
