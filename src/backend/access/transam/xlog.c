...
{
	return (LocalDataChecksumVersion > 0);
}
...
{
	return (LocalDataChecksumVersion == PG_DATA_CHECKSUM_INPROGRESS_VERSION);
}
...
SetDataChecksumsInProgress(void)
{
	...
	UpdateControlFile();
	LWLockRelease(ControlFileLock);
	WaitForProcSignalBarrier(EmitProcSignalBarrier(PROCSIGNAL_BARRIER_CHECKSUM_INPROGRESS));
}

void
AbsorbChecksumsInProgressBarrier(void)
{
	Assert(LocalDataChecksumVersion == 0);
	LocalDataChecksumVersion = PG_DATA_CHECKSUM_INPROGRESS_VERSION;
}
...
{
	Assert(LocalDataChecksumVersion == PG_DATA_CHECKSUM_INPROGRESS_VERSION);
...
show_data_checksums(void)
{
	if (LocalDataChecksumVersion == PG_DATA_CHECKSUM_VERSION)
		return "on";
	else if (LocalDataChecksumVersion == PG_DATA_CHECKSUM_INPROGRESS_VERSION)
		return "inprogress";
	else
		return "off";
}
...
xlog_redo(XLogReaderState *record)
{
	...
	else if (info == XLOG_CHECKSUMS)
	{
		...
		UpdateControlFile();
		LWLockRelease(ControlFileLock);
		if (state.new_checksumtype == PG_DATA_CHECKSUM_INPROGRESS_VERSION)
			WaitForProcSignalBarrier(EmitProcSignalBarrier(PROCSIGNAL_BARRIER_CHECKSUM_INPROGRESS));
		else if (state.new_checksumtype == 0)
			WaitForProcSignalBarrier(EmitProcSignalBarrier(PROCSIGNAL_BARRIER_CHECKSUM_OFF));
		else
			WaitForProcSignalBarrier(EmitProcSignalBarrier(PROCSIGNAL_BARRIER_CHECKSUM_ON));
	}
